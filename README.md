...
| 工具 | 免费额度到哪为止 | 核实于 |
|---|---|---|
| [DeepSeek](https://baipiaoji.com/tools/deepseek) | 网页与 App 端对话免费使用，支持文件上传与长文本对话；官方页面未公布具体条数上限，也未见任何会员/订阅档位（注：chat.deepseek.com 当前已有 Expert Mode / Instant Mode 的模式分档，各模式是否另有额度，官方页面未公布）。官方协议里明确的另一件事更重要：**输入与输出可用于个人使用、学术研究、衍生产品开发乃至训练其他模型（原文点名「模型蒸馏」）**——这个使用权范围在免费对话产品里相当宽松。同一份用户协议也写明输入与对应输出可能被用于模型训练与服务优化，用户可在产品内通过关闭「数据用于优化体验」选择退出。 | 2026-08-13 |
| [Kimi（月之暗面）](https://baipiaoji.com/tools/kimi) | 官方帮助中心明确：会话入口的主力模型（现为 K2.6）对所有用户免费、**不消耗会员额度**；单轮上下文约 128K tokens（官方口径约 5–6 万汉字）。免费档在官方会员体系里有正式名称——**Adagio（慢板）**，往上是 Andante（行板，49 元）与 Moderato（中速，99 元）。会员额度池是另一套：**所有高级功能共享同一个池**（Agent 系列、深度研究、PPT、文档、表格、Kimi Code、Kimi Work、Kimi Claw 等），按实际 token 消耗扣除，不是每个功能各给一份。 | 2026-08-12 |
| [通义千问](https://baipiaoji.com/tools/tongyi) | 开发者侧（阿里云百炼）官方明码：首次开通的新用户，平台主流模型**每款赠送 100 万免费 tokens**（输入+输出合计）；各模型、乃至同一模型的不同快照版本，免费额度相互独立，不合并、不转移、不共享。自 2025 年 9 月 8 日 11 点起首次开通百炼的用户，新人免费额度**有效期 90 天**（该时点前已开通的用户有效期不变，可能不足 90 天）；额度用尽或过期后自动失效，官方明示不支持补发、延期或重置。对话端（通义千问网页/App）免费使用，但对话轮次、文件大小、绘图张数等**免费档具体限制，官方文档仍未明示**；网络流传的排队、文件大小、绘图张数等数字出自阿里云开发者社区文章及第三方测评，非官方口径，本站不采信。另注：官方权益页面自称可调整权益形式、数量与价值，以当期页面展示为准。 | 2026-08-13 |
| [ChatGPT](https://baipiaoji.com/tools/chatgpt) | **2026-08-06 官方公告：免费档默认模型改为 GPT-5.6 Luna，并获得「无限日常文本对话」（unlimited text chats），另加一个用于难题的 Think 按钮。**时序要看清——官方原文说 Luna 成为免费档与 Go 档默认模型是「本周」，无限文本对话与 Think 按钮则是「下周起」，所以看到这条时未必已经全量到你账号。而且「无限」有两处明确的边界：一是**仅限文本**，文件上传、图片与其他工具的额度限制照旧；二是官方写明受**滥用防护（abuse guardrails）**约束。此前的口径仍然成立并继续适用于非文本部分：官方帮助中心明确「免费档的模型用量有限制」且不公布具体条数——官方口径是「达到上限时会在界面提示上限与重置时间」。未登录用户无法使用最新旗舰模型。任何第三方流传的「免费版每 X 小时 Y 条」都没有官方出处。 | 2026-08-07 |
| [Google Gemini](https://baipiaoji.com/tools/gemini) | 免费档按「算力用量」计量而非固定条数：官方明示用量取决于提示词的长度与复杂度、上传文件的数量与大小、对话长度、所用模型与功能。限额每 5 小时刷新一次，另有周度上限。 | 2026-08-13 |
...
| [Cloudflare Workers AI](https://baipiaoji.com/tools/cloudflare-workers-ai) | 每天 10,000 Neurons 免费（Neurons 是 Cloudflare 统一的推理计量单位，token/图块/音频分钟都折算进去），Workers Free 与 Paid 计划都含这份额度，每天 00:00 UTC（北京时间 08:00）重置。 | 2026-09-16 |
...
| [通义万相](https://baipiaoji.com/tools/tongyi-wanxiang) | 走阿里云百炼开通时自动发放新人免费额度，额度按「成功生成的输出图片数量」计，有效期 90 天（官方 FAQ 口径；具体张数官方未在可核实引文中给出，不猜测）。网页版每日赠送次数官方同样未公布。 | 2026-08-06 |
| [文心一格（百度）](https://baipiaoji.com/tools/yige) | 电量通过任务免费领取：首次登录、每日签到、分享任务等（官方口径明确机制）；单次任务的电量数额官方未公布，第三方数字不采信。 | 2026-08-06 |
| [Leonardo AI](https://baipiaoji.com/tools/leonardo) | 免费档每天发放 150 个快速 tokens，每 24 小时重置；当天没用完不结转、过期作废（官方帮助中心明码）。 | 2026-08-03 |
| [Ideogram](https://baipiaoji.com/tools/ideogram) | 免费档每周 10 个慢速积分——最多 10 次生成、约 40 张图，每周六 00:00 UTC 重置（官方文档明码）；慢速队列逐个排队处理，等待时长看平台负载。 | 2026-08-06 |
| [Krea AI](https://baipiaoji.com/tools/krea) | 免费档为每日补充的算力额度（用完等次日刷新），可用图像/视频/3D/口型的基础模型——机制为官方定价页明示，但具体数额官方未公布，第三方数字不采信。 | 2026-08-06 |
| [Vidu（生数科技）](https://baipiaoji.com/tools/vidu) | 免费档每月发放免费积分（官方页面同时出现 40 与 80 两种口径，具体以站内到账为准），另有错峰模式（Off-Peak）不消耗积分即可生成。 | 2026-08-06 |
| [PixVerse（爱诗科技）](https://baipiaoji.com/tools/pixverse) | 免费档每日赠送积分，官方博客口径为每天 30–60 积分、具体数额以应用内为准；生成结果带水印。 | 2026-08-02 |
| [智谱清影](https://baipiaoji.com/tools/qingying) | 免费版开放使用（发布时官方口径为「免费不限量」），生成需排队，单次约 5 秒、分辨率 1440×960，带水印。 | 2026-08-06 |
| [Luma Dream Machine](https://baipiaoji.com/tools/luma) | 免费档为限量试用积分（按月发放），官方页面未公布具体数额；第三方流传的「每天 5 次」在官方定价与帮助页均查不到，不予采信。生成结果带水印。 | 2026-08-03 |
| [Google Flow](https://baipiaoji.com/tools/google-flow) | **没有订阅的账号每天 50 个 Google Flow 积分**，官方帮助中心明码，用于试用；合格的 Workspace 套餐（Business Starter/Standard/Plus、Enterprise Starter/Standard/Plus、G Suite Legacy 免费版、Education Standard/Plus、Google AI Pro for Education）同样是 **50 积分/天**，不额外收费。免费积分**只能用于 Veo 3.1 的 Lite、Fast、Quality 三档生成**。各档消耗的积分数不同，但官方帮助页在可核实引文中没有列出每档具体数字，第三方文章流传的数字不予采信。 | 2026-08-06 |
| [飞书妙记](https://baipiaoji.com/tools/feishu-miaoji) | 基础版每个自然月、每位用户可累计使用 300 分钟妙记语音转文字（涵盖会议录制、录音、本地文件上传、云文件导入四个场景），每月 1 号恢复额度。**会中字幕不占用这份额度**。 | 2026-08-04 |
| [AiPPT](https://baipiaoji.com/tools/aippt) | 免费档官方三条明码：600MB 文件存储、每月 150 次 AI 生图、每天 1000 次 AI 助手调用。生成的作品可下载 PDF 或在线预览。 | 2026-08-04 |
| [Notion AI](https://baipiaoji.com/tools/notion-ai) | 免费与 Plus 计划的工作区可获得一份「体验用」的 AI 回复额度（官方帮助中心专页 complimentary AI responses），但**具体条数官方未在当前页面公布**——网上流传的「每人 20 次」出自 2023 年的活动、早已结束，不可采信。Notion AI 现按 $10/人/月 作为加购项提供（年付约 8 折）。 | 2026-08-04 |
| [OpenRouter](https://baipiaoji.com/tools/openrouter) | 免费账户可用 20+ 个 $0 模型（ID 带 :free）：每天 50 次请求、每分钟 20 次。一次性充值 $10 后，免费模型上限提升至每天 1,000 次（每分钟仍为 20 次），且该笔存款不过期、可用于付费模型。 | 2026-08-03 |
| [Cerebras](https://baipiaoji.com/tools/cerebras) | 新账号绑定并验证支付方式后获得 $5 免费额度，自发放起 30 天到期，可用于全部公开模型；免费档上下文长度 8,192 tokens。官方文档明确写明：目前不提供会自动续期的免费档，也没有任何模型的「永久免费」额度。 | 2026-08-05 |
| [Mistral AI](https://baipiaoji.com/tools/mistral) | API 免费档（Experiment 计划）真实存在且覆盖全系模型（含 Mistral Large、Codestral），无需绑卡，但要过两道门：手机号验证 + 同意输入数据用于模型训练。官方已不再公开发布具体限额数字，以控制台 Admin Console → Limits 的实时显示为准；常被引用的典型档位约 1 次/秒、500,000 tokens/分钟、10 亿 tokens/月。 | 2026-08-17 |
| [NVIDIA NIM](https://baipiaoji.com/tools/nvidia-nim) | 加入免费的 NVIDIA Developer Program 后，可免费调用 NVIDIA 托管的 NIM API 端点做开发与测试（官方措辞「unlimited prototyping」），无需信用卡，覆盖 DeepSeek / Llama / Qwen 等百余个模型。 | 2026-08-01 |
| [Cohere](https://baipiaoji.com/tools/cohere) | Trial（试用）API Key 每月 1,000 次调用免费，注册即得、无需绑卡。不同端点另有每分钟速率限制：Embed 每分钟 5 次、Chat 每分钟 20 次——月额度没用完也会被分钟级限速卡住。 | 2026-08-05 |
| [Hugging Face](https://baipiaoji.com/tools/huggingface) | 官方说明每个 Hugging Face 账号每月都会获得一笔 Inference Providers 额度用于试用，可跨多家推理服务商使用——但免费档的具体金额官方未在可核实页面写明。有明码的是 PRO：每月 $2 推理额度，官方另称 PRO 的包含额度是免费档的 20 倍。 | 2026-08-05 |
...
| [秘塔 AI 搜索](https://baipiaoji.com/tools/metaso) | 官方用户协议确认免费（普通）用户每日可领到一定额度的「刷新积分」，但官方明确写明「具体数额以服务页面显示为准」，未在任何可检索到的厂商页面公布具体数字。官方也从未说过「不限次数」——协议写的是按积分计费：普通模型回答 1 积分，DeepSeek 模型回答 3 积分，第一次工具调用不消耗积分、从第二次起每次 1 积分（消耗=(工具调用-1)+模型回答）。因此免费档是「有额度但官方不公布数字」，不是无限。 | 2026-08-13 |
| [Perplexity](https://baipiaoji.com/tools/perplexity) | 官方未公布可用的整体免费额度数字，且官方页面之间互相矛盾。现行帮助中心只写「免费档可使用顶级 AI 模型，但每周用量有限」，以及「免费档可以用 Best 和有限次数的 Pro Search，但不能手动选择高级搜索模型」——两处都明确承认「有限」却都不给数字，计量单位为「每周」。另有一篇旧版帮助文仍写着「每天 5 次 Pro Search、3 次文件上传」，与现行页面在两个维度上直接冲突（给数字 vs 不给数字、按天 vs 按周），且该文仍使用 Pro Search / sessions / Discover 这套旧产品词汇，判为未同步的过期页面，不予采信。官方博客另写过「免费用户每月 3 次」用于体验高级数据源集成的检索，但该数字只覆盖这一个子功能、不等于整体额度，且同属旧版结构期的发布公告，是否适用于现行套餐同样未获现行页面确认，因此本站不将其作为免费额度发布。关于「基础搜索不限次数」：官方自有页面上未见此说法，流传的 unlimited quick / basic searches 均出自第三方站，本站不予断言。 | 2026-08-13 |
| [NotebookLM（Google）](https://baipiaoji.com/tools/notebooklm) | 免费档官方明码四组数字，2026-08 复核仍是现行口径：最多 100 个笔记本、每本最多 50 个来源（单源上限 50 万词，本地上传单文件最大 200MB）、每天 50 次对话提问、每天 3 次音频概览（AI 播客）生成。注意：产品已官方更名为 Gemini Notebook，帮助中心迁到 support.google.com/gemininotebook（旧 /notebooklm/ 路径同 answer ID 仍可达），但这四个数字随更名未发生变化。官方未在自有页面公布免费档的视频概览 / 报告 / Deep Research 的具体次数——流传的「每天 3 次视频概览、每天 10 份报告、每月 10 次 Deep Research」只见于第三方博客，本站不采信。 | 2026-08-13 |
| [Felo Search](https://baipiaoji.com/tools/felo) | 官方现行免费额度未能确认，本站暂不给出具体数字。此前记录的「高速搜索（Quick Search）不限次数 + 专业搜索（Pro Search）每天 5 次」只见于 felo.ai 上的博客文章，与另一篇宣布个人订阅改为统一积分制、所有功能共用同一积分池的官方博客直接冲突——「不限次」与「共用积分池」不可能同时成立，说明旧表述至少已有官方未说明的边界。厂商既未下线旧页面，也未说明旧口径是否废止。同时，积分制公告中「Standard accounts 每天获得 200 积分」的 Standard 是否就是免费档，官方措辞未作界定，不能直接当作免费额度写入。本次复核无法访问 felo.ai 当期定价页（felo.ai/pricing、felo.ai/en/pricing），故按「官方现行数字未确认」处理：免费档的专业搜索次数与每日积分数均记为未确认，待能实读官方定价页当期免费档那一栏后再补。 | 2026-08-13 |
| [Consensus](https://baipiaoji.com/tools/consensus) | 免费档每月 15 条 Pro message（AI 综述，单次最多汇总 20 篇论文）+ 每月最多 3 次 Deep review（文献级综述，单次最多 50 篇论文）；官方特别说明：免费用户每跑一次 Deep，也会同时扣掉一条 Pro message 额度。 | 2026-08-13 |
| [Grok（xAI）](https://baipiaoji.com/tools/grok) | 免费档可用，但**官方不公布具体条数**——x.ai 定价页与 Grok 官方 FAQ 都只说「Grok 免费起步」，没有给出免费档每天/每周多少条。任何第三方流传的「免费版每 X 小时 Y 条」都没有官方出处，本站不转述。真正写明的是**额度的结构**：付费 SuperGrok 各档共用**一个按周结算的用量池**（跨产品共享，不是每个功能各给一份）。另有一个常见的张冠李戴要避开：**xAI 文档里能查到的速率限制（RPS/TPM）是 API 团队配额，不是网页版与 App 的免费额度**，两者不能互相代入——按 API 的 RPS 去推网页版每天能聊多少，得出的数字没有意义。 | 2026-08-08 |
| [微软 Copilot](https://baipiaoji.com/tools/ms-copilot) | 免费档每天 15 个 boosts（加速额度）用于生成图片，每 24 小时刷新；官方同时说明免费用户在非高峰时段可使用最新 AI 模型。boosts 用尽后仍可继续生成图片，只是速度变慢——官方未对免费出图给出每日总量上限。 | 2026-08-08 |
| [Poe](https://baipiaoji.com/tools/poe) | 官方未公布免费档的具体额度。Poe 帮助中心（help.poe.com）只说明机制：非付费用户每天获得一批计算积分（compute points），积分在每 24 小时周期结束时重置、不结转，订阅用户获得的积分数高于免费用户。在可检索到的官方页面文本（帮助中心 FAQ、poe.com/subscription_plans）中，均未出现免费档「每日 N 积分」的固定数字，实际余额只在产品界面内显示。不同模型每条消息扣的积分差异很大，因此官方也没有给出免费档能发多少条消息。（第三方口径从 300 到 3000 互相打架，一概不采信。）

配套 wall 建议改为：积分耗尽后无法继续向消耗积分的模型发消息；帮助中心的处理口径是「不结转，需要更多就等次日续发，或现在付费购买」，官方博客与购买 FAQ 另给出加购积分约 $30/100 万积分的换算。订阅档位价格与各档积分数不写入本条——官方定价页当前同时呈现「starting at $4.99/month」与年付折算「$4.17/mo」两种起价口径，且无法核实旧的 $9.99 档是否仍在售；原记录中的「每日 10,000 积分」只见于第三方，直接删除，不用新价替换。 | 2026-08-13 |
| [GitHub Models](https://baipiaoji.com/tools/github-models) | 免费用量按模型分档限流（RPM/RPD/单次 tokens/并发四个维度，官方文档明示机制）：常被引用的典型档位为低档模型约 150 次/天（如 GPT-4o mini 档）、高档约 50 次/天（如 GPT-4o 档）；各模型实时限额以官方 rate limits 表为准。 | 2026-08-03 |
| [MarsCode（字节）](https://baipiaoji.com/tools/marscode) | 产品线已合并：豆包 MarsCode 编程助手于 2025 年正式升级为「Trae 插件」，原网站并入 Trae 入口、账号体系互通（字节官方公告）；免费权益随 Trae 体系走，插件形态的基础补全与问答免费。 | 2026-08-03 |
...
| [无界 AI](https://baipiaoji.com/tools/wujie) | 新用户 AI 魔法体验卡每日 0 点补足 15 次免费创作（官方文章明码）；积分获取同样官方明码：注册 +50、实名认证 +100、绑卡 +150、邀请注册 +20/人（实名 +30），2 积分兑 1 次生成（通用/动漫/色彩模型）。 | 2026-08-06 |
| [Bing 图像创作](https://baipiaoji.com/tools/bing-image) | 官方帮助页口径：每天 15 次「快速生成」免费，用完后快速生成需消耗 Microsoft Rewards 积分（约 10 分一次）；**标准速度生成始终不限量且免费**——这条最实用，不赶时间就没有额度概念。 | 2026-08-06 |
| [Playground AI](https://baipiaoji.com/tools/playground-ai) | 免费档官方明码三条：自家模型每 3 小时可做 10 次图像编辑（滚动窗口，随时间逐步回血、不是每日一刀切）；接入的第三方高级模型每月仅 3 次；**下载每天 10 张**。 | 2026-08-06 |
| [Civitai](https://baipiaoji.com/tools/civitai) | 免费白嫖 Buzz 的三条官方路径：每日登录打开生成器即送；浏览带广告页面被动攒蓝 Buzz（0.25/条广告，上限 100/小时、400/天）；每日任务最多再赚 225/天。消耗侧：SD1.5 一张约 1–2 Buzz、SDXL 约 4–6，草稿模式半价（均为官方文章口径）。 | 2026-08-06 |
| [remove.bg](https://baipiaoji.com/tools/removebg) | 网页版抠图预览分辨率（最高 0.25 兆像素，约 625×400）不限次免费下载；注册送 1 个积分可测试一次高清导出；API 与桌面/插件端每月另有 50 次免费预览调用（官方帮助中心明码）。 | 2026-08-06 |
| [Upscayl](https://baipiaoji.com/tools/upscayl) | 后端 AGPLv3 开源、完全免费、无额度概念：全部张量计算通过 Vulkan 在本机 GPU 上跑，不联网、不上传，因此不限张数也没有隐私顾虑。最高可放大到 16 倍，内置 Real-ESRGAN、Remacri、Ultramix 等多种模型分别适配照片/纹理/2D 插画。 | 2026-08-06 |
| [Pixlr](https://baipiaoji.com/tools/pixlr) | 基础在线修图免费使用（带广告）；新用户赠送 20 个 AI 积分、约生成 20 张图（官方博客口径）。免费档另有保存次数限制——机制官方明示，但具体数字未公布，不采信第三方口径。 | 2026-08-06 |
| [吐司 AI](https://baipiaoji.com/tools/tusiart) | 社区端可免费在线出图（支持 SD 1.5 / SDXL / 混元 DiT 等底模与 LoRA、ComfyUI 工作流），但**每日免费算力的具体数额官方未在可核实页面公布**，不写数字。开发者侧 API 官方明码：算力单价 0.01 元/点，无需自建 SD 环境与显卡。 | 2026-08-06 |
| [Pika](https://baipiaoji.com/tools/pika) | 免费档可用 Pika 2.5 模型、分辨率限 480p，含 Pikascenes、Pikadditions、Pikaswaps 等特效（Pikaffects 仅图生视频）——以上为官方定价页明示；具体积分数额官方页未公布，第三方流传的数字一律不采信。 | 2026-08-06 |
| [Haiper](https://baipiaoji.com/tools/haiper) | 可免费试用视频生成（官方口径为「一定数量」，**每日/总量的具体数额未在可核实页面明示**，因此不写数字）；官方给出的消耗参考：生成 5 秒视频需 50 积分。官方文档同时说明因新功能上线，定价结构正在调整。 | 2026-08-06 |
| [OpusClip](https://baipiaoji.com/tools/opusclip) | 免费档（free-forever）每月 60 分钟视频处理时长，按月刷新；新用户另有 7 天 Pro 试用（约 90 分钟处理时长）。 | 2026-08-06 |
| [HeyGen](https://baipiaoji.com/tools/heygen) | 免费档每月最多 3 条视频、1080p，另含 Avatar IV、Video Agent、口型翻译等高级功能的有限试用；注册无需绑卡。所有免费档导出**带水印**。 | 2026-08-06 |
| [D-ID](https://baipiaoji.com/tools/d-id) | 免费试用赠送 12 个积分，每个积分对应最多 15 秒视频（合计约 3 分钟）；试用期 14 天，试用结束前取消不扣费。积分**不累积**，按月刷新、未用完即作废。 | 2026-08-06 |
| [Udio](https://baipiaoji.com/tools/udio) | 免费档每天 10 积分，用完后可从每月 100 积分的补充池扣（官方帮助中心明码）；1 积分对应约 30 秒音轨，每次生成同时出 2 条即耗 2 积分；积分一律不结转。 | 2026-08-06 |
| [TTSMaker](https://baipiaoji.com/tools/ttsmaker) | 免费档每周 20,000 字符，部分标注「不限量」的音色不计入周额度、可无限使用；官方设有专门的商用许可条款页，明确免费生成的音频可商用、无需额外授权。 | 2026-08-02 |
| [ProcessOn](https://baipiaoji.com/tools/processon) | 免费账号可编辑的文件数为 9 个（按最近使用计）；超出 9 个的旧文件仍可查看与下载，但不能继续编辑。 | 2026-08-04 |
| [Napkin AI](https://baipiaoji.com/tools/napkin) | 免费档面向个人与小团队：可用核心功能，AI 生成积分有限（官方未公布具体数额，第三方数字不采信），导出选项为基础档。 | 2026-08-03 |
| [歌者 PPT](https://baipiaoji.com/tools/gezhe) | 官方公告口径：**永久免费**生成 PPT，且**可免费下载 PPTX 源文件**——这一条正是 AiPPT 免费档做不到的（后者只能导 PDF 或在编辑器内导出）。功能含任意话题一键生成、导入资料转 PPT、PPT 智能翻译。 | 2026-08-04 |
| [腾讯 ima](https://baipiaoji.com/tools/ima) | 免费提供 1GB 个人云存储（官方口径约可存 200 份标准论文）。另有三条免费扩容路径：把知识库发布到「知识库广场」共享可获**免费无限扩容且不占个人空间**；广场里别人的内容同样不占你的空间；通过邀请新用户与活动 CDkey 累计**最高可得 100GB**。 | 2026-08-04 |
| [沉浸式翻译](https://baipiaoji.com/tools/immersive-translate) | 基础翻译引擎（微软/谷歌等）免费不限次；登录后可领取 5 万 AI 高级模型 tokens（官方入门页口径），用于 ChatGPT/DeepL/Gemini 等 20+ 引擎的高级翻译；也可自带各家 API Key 走自己的额度。 | 2026-08-03 |
| [有道翻译](https://baipiaoji.com/tools/youdao) | 开发者侧（有道智云）明码：新注册账号系统赠送 50 元体验金，先扣体验金、用尽后按接口调用量计费。云笔记侧提供 3GB 免费存储空间。 | 2026-08-04 |
| [Wordvice AI](https://baipiaoji.com/tools/wordvice) | 免费 Basic 档（需注册）：改写单次最多 500 词；AI 翻译单次最多 500 字符；各修订工具合计可处理 5,000 词文本（均为官方 plans 页口径；官方未明示 5,000 词的刷新周期，故不写周期）。 | 2026-08-03 |
| [夸克 AI](https://baipiaoji.com/tools/quark-ai) | 官方未公布任何具体数字。夸克官网与各 AI 产品页（AI 搜索、AI 写作、AI PPT、AI 生图、深度思考）只用「免费」「免费生成」这类措辞描述免费档，从未写出每日/每月可用次数、可用条数、积分或点数余额，也没写额度按什么周期重置；付费的「夸克 AI 会员 / 网盘 SVIP」页面只罗列付费能享受什么，不写免费档到哪为止。第三方广泛流传的「AI PPT 每天 3 次」「AI 搜题免费不限次」等说法，全部追不到夸克自有页面的原始出处，本站不采信。夸克以移动端为主，若真存在次数上限，很可能仅在 App 内以弹窗形式提示，网页侧无从核实。 | 2026-08-13 |
| [360 AI 搜索](https://baipiaoji.com/tools/ai360) | 官方未公布任何具体数字。360 AI 搜索的基础搜索与问答可免费使用，但在 360 自有域名上找不到任何写明免费用户每天／每月能用多少次的页面：官网、帮助中心、协议页均无免费档的次数、条数或额度公示，也不存在一个列出免费档与付费档对比的官方定价页。搜索索引中可见的 360 自有协议文本只从付费侧描述权益（会员可享「不限次数的AI分析」），并明写权益细节「以您实际购买时的产品界面展示及服务权益实际展示为准」，即具体数字只存在于登录后的产品界面。需要说明的是，《360AI大会员自动续费服务协议》中「每个AI应用，每天的使用次数有限制」一句出自付费大会员协议、描述的是会员侧的权益边界，不能据此断言官方已就免费档公布或承认上限——免费档究竟有没有上限、上限是多少，官方一律未公布。流传的「文档5份/天」「翻译1万字/天」及各类会员价格均出自第三方站点，追不到 360 自有页面，按规则不采信。 | 2026-08-13 |
| [知乎直答](https://baipiaoji.com/tools/zhihu-zhida) | 官方未公布免费档的任何额度数字。知乎从未为「知乎直答」发布过定价页、额度页或帮助中心条目：搜索索引中的官网（zhida.zhihu.com）引文通篇只讲产品能力（AI 搜索、内容溯源、知识库），没有提问次数、对话轮数、每日或每月上限；官方也从未出现「不限次数／无限使用」的表述——即「到底有没有限制」这件事本身，官方都没写过。网上流传的知识库容量 1GB／5GB／50GB 三个互相冲突的版本，以及「最多上传 100 个文件、单个文件最大 100 MB」的上传规则，均只见于搜索索引摘要或知乎用户专栏（zhuanlan.zhihu.com 属 UGC），既无法确认现行有效（有迹象表明该上传规则的索引快照早于后续知识库扩容），也不属于免费档使用额度，故一律不作为额度采用。 | 2026-08-13 |
| [Devv AI](https://baipiaoji.com/tools/devv) | 免费档按月发放积分（官方文档确认月度额度机制与积分制，但免费档具体数额未在可核实引文中明示，不猜测）；积分主要被编程智能体消耗。 | 2026-08-04 |
| [Phind](https://baipiaoji.com/tools/phind) | 官方未公布：追不到任何 phind.com 自有页面写明免费档的具体次数，因此不填数字。网上流传的「每天 5 次 / 10 次 / 500 次 Phind-405B / 1000 次 research / Phind Instant 无限」等说法全部出自第三方站点，彼此矛盾，其中多项被描述为 Pro 付费档限额而非免费档，另有一批明显来自 Plus $15、Pro $30 的旧版定价结构，按本站规矩一律不采信。另需提示：多家第三方来源（Hacker News 讨论页转述的创始人公告、X 用户贴文、工具目录站）称 Phind 已于 2026 年 1 月 16 日停止服务，但该消息同样追不到 phind.com 自有页面，我们无法核实；本站 2026-08-13 的可达性探测对 www.phind.com 得到的是 HTTP 403 应答（服务器有响应，并非域名失效），因此我们既不宣称它已停运，也不宣称它仍在正常运营——在拿到厂商自有页面的说明之前，这里只写"官方未公布免费额度"这一件已确认的事。 | 2026-08-13 |
| [Fireworks AI](https://baipiaoji.com/tools/fireworks) | 新用户注册自动获得 $1 免费额度（约合 70B 级模型 100 万 token），无需绑卡即可开始调用。 2026-09-12 复核：官方文档仍写新账户自动获得 $1 免费额度；另据官方博客，2026-07-01 起自助账户已改为预付费（按余额扣减），$1 启动额度不变、用完即按余额计费，它是一次性启动金，不是持续免费层。 | 2026-09-12 |
| [阿里云百炼](https://baipiaoji.com/tools/bailian) | 新用户每款模型各赠 100 万免费 Tokens（不同模型、乃至同一模型不同版本的额度相互独立，不互通不共享），覆盖平台 70+ 款模型；官方帮助中心明示有效期为开通日起 90 天。官方文章对「总额度」口径不一（超 5,000 万与超 7,000 万并存），故不取总数。 | 2026-08-03 |
| [DeepInfra](https://baipiaoji.com/tools/deepinfra) | 通过 Hugging Face Inference Provider 接入时，免费用户享有一份每月小额配额（官方博客口径，未给具体数值）；DeepStart 创业计划可申请最高 10 亿 tokens 的免费推理额度，需申请审核。 | 2026-08-04 |
...
| [Motiff 妙多](https://baipiaoji.com/tools/motiff) | **⚠ 该产品已停止服务，本条目仅作存档与迁移指引，不再是可用的免费额度。** 官方关停说明：因业务调整停运；**官方两处日期不一致**——英文帮助页写 2026-06-23，中文「妙多关停说明」写 2026-07-31，本站两个都列、不替厂商选，但无论按哪个，截至本条核实日均已过期。2026-05-13 起已关闭新注册与续订，存量账号可登录使用至关停日。付费用户按官方口径自动退款（月付全额退最近一期并加赠一个月、年付全额退年费并加赠一个月），退款于 2026-05-31 前处理。 | 2026-08-06 |
| [稿定 AI](https://baipiaoji.com/tools/gaoding) | 在线设计与编辑功能可免费使用，模板与素材分为免费与会员两类（会员分模板会员、素材下载会员、大会员三档）。免费档具体下载额度官方未在可核实页面明示，因此不写数字。 | 2026-08-04 |
| [Framer](https://baipiaoji.com/tools/framer) | 免费档官方明码：10 个 CMS 集合、1,000 页、单文件上传 5MB、1 个免费语言区域可试。站点发布在 framer.website 免费域名下，并带「Made in Framer」标识。 | 2026-08-06 |
| [Elicit](https://baipiaoji.com/tools/elicit) | Basic 免费档三项不限量：检索 1.38 亿+ 篇论文、跨论文摘要、与有全文的论文对话；Research Agent 与研究报告走按月重置的共享用量池，免费档池内具体数额官方未明示（不猜测）。 | 2026-08-04 |
| [SciSpace](https://baipiaoji.com/tools/scispace) | 免费档官方三条明码：每份 PDF 最多 120 页、每天 3 份 PDF、每天 50 个提问。Plus 档 $5/月（每份 2000 页、每天 50 份、每天 1000 问）作为对照。 | 2026-08-04 |
| [豆包爱学](https://baipiaoji.com/tools/doubao-aixue) | 登录即可免费使用（官方口径：登录即免费使用豆包提供的各项功能，爱学属其学习辅助方向）。**具体使用次数或额度官方未在可核实页面公布**，因此不写数字。 | 2026-08-04 |
...
| [LanguageTool](https://baipiaoji.com/tools/languagetool) | 两条产品线两套额度：编辑器与浏览器插件的免费版单次检查上限 10,000 字符（Premium 提到 100,000）；公开 HTTP API 免费口径为每 IP 20 次/分钟、75,000 字符/分钟、单次请求 20,000 字符（Premium API 为 80 次/分钟、300,000 字符/分钟、单次 60,000）。 | 2026-08-17 |
| [Exa](https://baipiaoji.com/tools/exa) | Exa 免费档发的是「额度券」不是请求数：搜索索引中出自官方定价页的引文写明新账号注册赠送 $20 免费额度（官方括注约合 2,800 次搜索，与其 Search $7/1k 的单价吻合：20 ÷ 7 × 1000 ≈ 2857），之后免费档每月再补 $10 额度。Websets 侧官方另写免费方案含 1,000 credits（每个 webset 最多 25 条结果、功能受限）。官方页面从未出现第三方站广泛流传的「每月 20,000 次免费请求」这一说法，该数字不予采信。

【时效性存疑，请以官网当前页面为准】以上数字均来自搜索引擎索引的引文，本次核实环境无法直接打开 exa.ai 页面，**引文的快照日期不明**。有第三方记录（不采信为来源，仅作为提醒）称 Exa 在 2026 年 7 月调整过免费档；同时流传中还存在另一套与上述不同的赠额结构（「$10 注册赠额 + 每月 $7」），两套结构不可能同时现行，说明其中至少有一代已是旧版定价。我们未能确认上文引用的是否为现行版本，因此此处不声称这些数字「当前有效」，仅记录它们确实出自 Exa 自有页面。 | 2026-08-13 |
| [GPTZero](https://baipiaoji.com/tools/gptzero) | 免费档每月 10,000 词、单次扫描上限 10,000 字符，无需绑卡。 | 2026-08-17 |
| [Ante](https://baipiaoji.com/tools/ante) | 工具本身不设额度：预览版二进制免费，官方 Binary Preview Terms 明示包含商用。模型走自带 Key（BYOK），支持 12+ 家供应商（Anthropic、OpenAI、Google、xAI、OpenRouter 等），所以推理花多少完全等于你自己那把 Key 的价钱，本站不做任何估算。本条独有的一种形态：内置 llama.cpp，跑本地模型可完全离线——这种用法下根本不需要任何 Key。 | 2026-08-17 |
